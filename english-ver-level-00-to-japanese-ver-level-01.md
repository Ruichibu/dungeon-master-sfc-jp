...
* **E01 [The Door of Champions]**
  * **発動条件:** セロンが勇者の館で勇者を1人以上「復活（Reincarnate）」または「転生（Resurrect）」させ、実体のあるパーティを結成する。
  * **ギミック:** そのパーティの状態で `(L00, 06, 09)` の床スイッチ（X）を踏むと、`(L00, 05, 09)` の通常ドア（D）が永久に開放される。これにより、館の裏側（西側エリア）への道が開ける。
