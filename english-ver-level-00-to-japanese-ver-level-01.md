...
* **G01 [Game Start]**
  * プレイヤーは `(L00, 01, 03)` から肉体のないスピリット状態でスタート。
* **E01 [The Door of Champions]**
  * **発動条件:** 勇者の館で勇者を1人以上「復活（Reincarnate）」または「転生（Resurrect）」させ、実体のあるパーティを組む。
  * **ギミック:** その状態で `(L00, 06, 09)` の床スイッチ（X）を踏むと、`(L00, 05, 09)` の通常ドア（D）が永久に開放される。これで館の裏側のエリア（西側）へ探索を進めることが可能になる。
* **E02 [To the Next Level]**
  * `(L00, 04, 15)` にある下り階段（S）を進むと、次の階層である `(L01, 03, 14)` の上り階段へと移動する。
